...
async function validate(body){
  ...
  if(!userName){    result.error = true; result.messages.push("No name provided") }
  if(!role){ result.error = true; result.messages.push("No address provided") }
  if(!(role=='admin'||role=='client'||role=='worker')){
    result.error = true; result.messages.push("Role sholud be either client or worker")
  }
  return result
}
